...
2026-09-26 live growth recheck: release `5c24e12` is deployed. Production currently returns 4 fresh Splinterlands strategies from one opportunity; the homepage displays one opportunity-diversified result. The 14:44 UTC manual recalculation fell in the same 12:00–16:00 UTC cadence/idempotency window as the existing 13:08 snapshot, so it reused the same records. The Farmers World derived-freshness correction is live but awaits verification in a new cadence window beginning 16:00 UTC. Refresh policy still skips three DFK strategies (no positive cJEWEL balance), Storj (no approved loader), and four partial DePIN models (required economics absent). The strategy history now requests the newest page. See `docs/GROWTH_OPERATING_PLAN.md` for acquisition evidence and priorities.
