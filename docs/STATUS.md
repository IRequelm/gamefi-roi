...
2026-09-26 live growth recheck: releases `5c24e12` and `68c7141` are deployed; homepage visitor-path patch `f4e2a1c` also went live at 15:22 UTC. Production at 15:17 UTC still returned 4 fresh Splinterlands strategies from one opportunity; the last persisted calculation was 13:08 UTC. The Farmers World derived-freshness correction awaits a calculation in the new window beginning 16:00 UTC. Refresh policy still skips three DFK strategies (no positive cJEWEL balance), Storj (no approved loader), and four partial DePIN models (required economics absent). The strategy history now requests the newest page. A catalog search/type filter is being added to make the 51-opportunity catalog easier to navigate. See `docs/GROWTH_OPERATING_PLAN.md` for acquisition evidence and priorities.
