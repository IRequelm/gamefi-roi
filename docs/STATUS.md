...
2026-09-26 live growth recheck: release `822b6a1` is deployed and verified on `https://gamcryp.com/`; its homepage distinguishes fresh strategies from prior models without current data. A follow-up catalog SSR/hydrated freshness correction is tested locally but not deployed. Search/type filters preserve all 51 catalog records. One manual recalculation for the 16:00–20:00 UTC window (GitHub Actions run `36255076672`) completed at 16:20 UTC with 7 new snapshots; the recovery check and production API both show `2026-09-26T16:20:27Z`, status `ok`, 7/7 eligible strategies fresh, 8/15 total strategies stale or structurally unrefreshable, zero unresolved failures, and only two opportunities with current results. The 16:17 scheduled run had not appeared by 16:20, so sustained four-hour schedule reliability is unverified. Refresh policy excludes three quarantined DFK strategies (RPC source/balance not validated), Storj (no approved live economics loader; October payout denomination changes to USDC), and four partial DePIN models (required economics absent). Consented browser identity now joins its `/go` redirect event in PostHog; unconsented traffic remains anonymous. See `docs/GROWTH_OPERATING_PLAN.md` for acquisition evidence and priorities.
