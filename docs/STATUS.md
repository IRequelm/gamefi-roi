...
2026-09-26 live growth recheck: releases `5c24e12`, `68c7141`, `f4e2a1c`, and `8131aa0` are deployed. Live catalog search/type filtering was verified; this uncovered and fixed a hydration bug where the API's default 50-item page hid the 51st catalog entry. The fix, `fef4140`, is also deployed and live verification now preserves all 51 records (DIMO search 1/51; Games filter 14/51). Production `/api/v1/ops/status` at 15:37 UTC still showed 4/7 eligible strategies fresh, all Splinterlands, 11/15 modeled strategies stale, and the last persisted calculation at 13:08 UTC. Verify Farmers World derived freshness after the next four-hour window's scheduled run (expected near 16:17 UTC). Refresh policy still skips three DFK strategies (no positive cJEWEL balance), Storj (no approved live economics loader), and four partial DePIN models (required economics absent). See `docs/GROWTH_OPERATING_PLAN.md` for acquisition evidence and priorities.
