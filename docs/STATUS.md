...
2026-09-26 live growth recheck: releases through `fef4140` and the consent-scoped analytics identity fix `23139f0` are deployed. Catalog search/type filtering preserves all 51 records after hydration (DIMO search 1/51; Games filter 14/51). One manual recalculation for the 16:00–20:00 UTC window (GitHub Actions run `36255076672`) completed at 16:20 UTC with 7 new snapshots; the same-run recovery check and live production API both show `2026-09-26T16:20:27Z`, status `ok`, 7/7 eligible strategies fresh, 8/15 total strategies stale or structurally unrefreshable, and zero unresolved failures. The expected 16:17 scheduled run had not appeared by 16:20, so sustained four-hour schedule reliability is unverified. Refresh policy still excludes three quarantined DFK strategies (RPC source/balance not validated), Storj (no approved live economics loader; October payouts switch to USDC), and four partial DePIN models (required economics absent). The homepage UI review found stale modeled opportunities labelled “ROI can be measured”; a correction that labels freshness from current ranking data is tested locally but not deployed yet. A labeled production smoke request verified that consented browser identity now joins its `/go` redirect events in PostHog; unconsented traffic remains anonymous. See `docs/GROWTH_OPERATING_PLAN.md` for acquisition evidence and priorities.
...
Growth recheck (2026-09-26): last-28-day GA4 = 58 sessions / 4 active users (25 sessions explicitly test/manual; direct remainder is unclassified), Search Console = 162 returned page impressions / 0 clicks, YouTube = 215 views / 60 engaged views / 0 subscribers gained. Discovery scheduled task is succeeding. Distribution task/process is stopped; X remains disabled. YouTube Shorts worker is alive and has used its one-success daily publication cap. The next source change removes an unsupported device/time-fit claim from the homepage; pending backend web and frontend tests, production deploy, and rendered-page verification. V1 remains in OPERATIONS / GROWTH MODE and the acquisition goal is not complete.
