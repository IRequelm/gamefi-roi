...
2026-09-26 live growth recheck: commits `822b6a1` and `6cd2812` are deployed and verified on `https://gamcryp.com/`; the homepage and full catalog distinguish fresh strategies from prior models without current data. Search/type filters preserve all 51 catalog records. One manual recalculation for the 16:00–20:00 UTC window (GitHub Actions run `36255076672`) completed at 16:20 UTC with 7 new snapshots; at 16:47 UTC production remained `ok`, 7/7 eligible strategies fresh, 8/15 total strategies stale or structurally unrefreshable, zero unresolved failures, and only two opportunities with current results. The four-hour cadence has not yet run its next scheduled window. A core Farmers World capital bug was then found: 3- and 10-Axe strategies used one NFT floor listing as the entire basket cost. Commit in progress queries the required number of distinct cheapest listings and fails closed on insufficient depth. It must be deployed and recalculated before current Farmers World capital/ROI figures can be relied on. Refresh policy continues to exclude three quarantined DFK strategies (RPC source/balance not validated), Storj (no approved live economics loader; October payout denomination changes to USDC), and four partial DePIN models (required economics absent). Consented browser identity now joins its `/go` redirect event in PostHog; unconsented traffic remains anonymous. See `docs/GROWTH_OPERATING_PLAN.md` for acquisition evidence and priorities.
