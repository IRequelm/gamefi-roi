...
| Content quality | Representative frame QA, approved product visual, approved narration, checksum-bound human approval | FAIL-CLOSED; twelve Shorts now have v8 asset-led renders with music-only audio and no new ElevenLabs usage. Automated media/frame QA passes and the product capture/header treatment was revised for mobile readability, but all items remain `PENDING_HUMAN_REVIEW` until full renders and representative frames receive explicit checksum-bound approval; stale historical assets remain review-only |
